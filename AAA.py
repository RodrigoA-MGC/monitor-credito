...
def calcular_indicadores(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df.loc[mask_credito, "Uso_vs_Limite"] = (
        (df.loc[mask_credito, "Saldo vencido"] + df.loc[mask_credito, "Saldo por vencer"])
...
def mostrar_ficha_cliente(
    ...
    with tabs[0]:
        ...
        st.dataframe(
            snap_cli[[
                "Destinatario mercancia", "Condiciones de pago",
                "Saldo vencido", "Saldo por vencer",
                "Anticipos", "Depósitos SAP", "Límite de credito",
    ...
    for i, dest in enumerate(dests):
        with tabs[i + 1]:
            ...
            st.caption(f"Última fecha de corte: **{snap_d['fecha'].strftime('%d/%m/%Y')}**")
...
#  SESSION STATE
# ─────────────────────────────────────────────
for key in ("snapshot", "historico", "fuente_activa"):
...
st.dataframe(
    df_clientes[mask][[
        "Cliente", "Nombre", "Num_Destinatarios",
